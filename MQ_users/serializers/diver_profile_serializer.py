...
class DiverProfileSerializer(serializers.ModelSerializer):
    ...
    def update(self, instance, validated_data):
        ...
        # Mise à jour des champs de DiverProfile
        ...
        if emergency_contact_data:
            # Si le profil de plongeur a déjà un contact d'urgence, mettez-le à jour
            if instance.emergency_contact:
                for key, value in emergency_contact_data.items():
                    setattr(instance.emergency_contact, key, value)
                instance.emergency_contact.save()
            # Sinon, créez un nouveau contact d'urgence pour ce profil de plongeur
            else:
                EmergencyContact.objects.create(diver_profile=instance, **emergency_contact_data)
